...
/**
 * Upload implementation class person
 */
public class personUpload {
	...
	public static String DOCUMENTLIBRARY_PORTLETFOLDER = "person";
	...
	public person uploadFiles(ActionRequest request,person person) throws PortalException, SystemException, IOException, SecurityException, IllegalArgumentException, NoSuchMethodException, IllegalAccessException, InvocationTargetException{
			...
			if(strType.equalsIgnoreCase(DOCUMENTFILE)) {
				...
				if(deleteds.get(formField)!=null){
					...
					FileEntry fileEntry = PortletFileRepositoryUtil.addPortletFileEntry(
								groupId, userId, person.class.getName(), 
								person.getPrimaryKey(), portletName, 
								folderId, inputStream, sourceFileName, StringPool.BLANK, 
								true);					
				
					callSetMethod(formField,person,fileEntry.getFileEntryId());
					...
					if(hiddens!=null){
						Long prevDocument = (Long)hiddens.get(HIDDEN+SEPARATOR+formField);
						if((prevDocument!=null)&&(prevDocument!=0L)) {
							callSetMethod(formField,person,(Long)hiddens.get(HIDDEN+SEPARATOR+formField));
						}
					}
				}
			}
		...
		if(dlFolderId!=0L){
			person.setFolderDLId(dlFolderId);
		}
		return person;
	}

	private void callSetMethod(String formField, person person, Long value) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException{
		String strMethod = "set"+StringUtil.upperCaseFirstLetter(formField);
		Method methodSet = person.getClass().getMethod(strMethod,long.class);
		methodSet.invoke(person, value);
	}
